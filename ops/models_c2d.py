    ...
    def _prepare_base_model(self, backbone):
        ...
        if 'resnet' in backbone:
            if self.non_local:
                ...
                import nets.C2D_NLN as C2D_NLN
                self.base_model = getattr(C2D_NLN, backbone)(pretrained=True if self.pretrain == 'imagenet' else False)
            elif self.element_filter:
                import nets.C2D_ECals as C2D_ECal
                base_model_name = 'nets.C2D_' + self.net
                print('=> base model: {}, with backbone: {}'.format(base_model_name, backbone))
                # C2D_EF = import_module(base_model_name)
                block = 'EF_%s'%self.net
                # print(block)
                self.base_model = getattr(C2D_ECal, backbone)(pretrained=True if self.pretrain == 'imagenet' else False, EF=block,
