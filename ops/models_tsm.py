...
class VideoNet(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                ...
                temporal_pool=False, non_local=False, element_filter=False, cdiv=2, target_transforms=None):
    ...
    def _prepare_base_model(self, backbone):
        if 'resnet' in backbone:
            if self.non_local:
                ...
                import nets.TSM_R_ECals as TSM_R_ECal
                # assert self.net in ['E33D', 'E31D', 'T33D', 'T31D', 'S33D', 'S31D', 'CLLD']
                base_model_name = 'nets.TSM_R_' + self.net
                print('=> base model: {}, with backbone: {}'.format(base_model_name, backbone))
                block = 'EF_%s'%self.net
                # TSM_R_EF = import_module(base_model_name)
                self.base_model = getattr(TSM_R_ECal, backbone)(pretrained=True if self.pretrain == 'imagenet' else False, EF=block,
                                n_segment=self.num_segments, fold_div=self.shift_div, place=self.shift_place, cdiv=self.cdiv)
            else:
                print('=> base model: TSM_R, with backbone: {}'.format(backbone))
                import nets.TSM_R as TSM_R
                self.base_model = getattr(TSM_R, backbone)(pretrained=True if self.pretrain == 'imagenet' else False,
                                n_segment=self.num_segments, fold_div=self.shift_div, place=self.shift_place)
            #
